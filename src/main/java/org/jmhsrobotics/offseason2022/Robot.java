...
public class Robot extends LoggedRobot {
  ...
  private void initLogging() { // TODO: Change from default
    ...
    if (isReal()) {
      ...
      // String path = ByteLogReplay.promptForPath(); // Prompt the user for a file
      // path on the command line
      // Logger.getInstance().setReplaySource(new ByteLogReplay(path)); // Read log
      // file for replay
      // Logger.getInstance().addDataReceiver(new
      // ByteLogReceiver(ByteLogReceiver.addPathSuffix(path, "_sim")));

      // Save replay results to a new log with the "_sim" suffix

      Logger.getInstance().addDataReceiver(new ByteLogReceiver("./logs/")); // Log to USB stick (name will be
      // selected automatically)
      Logger.getInstance().addDataReceiver(new LogSocketServer(5800)); // Provide log data over the network, viewable in
      // Advantage Scope.
    }
